...
export const FavoriteToggle: FunctionComponent<FavoriteToggleProps> = ({ job }) => {
  const { id } = job;
  ...
  const handleClick = () => {
    setIsPressed((currentIsPressed: boolean): boolean => !currentIsPressed);
  };
  // End of TODO

  return (
    <button onClick={handleClick} className={cn(styles.button, { [styles.pressed]: isPressed })}>
